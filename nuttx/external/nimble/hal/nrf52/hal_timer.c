...
nrf_timer_set_ocmp(struct nrf52_hal_timer *bsptimer, uint32_t expiry)
{
    ...
    if (bsptimer->tmr_rtc) {
         ...
         */
        if (delta_t < 3) {
            hal_NVIC_SetPendingIRQ(bsptimer->tmr_irq_num);
        } else  {
            if (delta_t < (1UL << 24)) {
...
hal_timer_init(int timer_num, void *cfg)
{
    ...
    hal_NVIC_SetPriority(irq_num, NVIC_SYSH_MAXNORMAL_PRIORITY + NVIC_SYSH_PRIORITY_STEP);
    hal_NVIC_SetVector(irq_num, (uint32_t)irq_isr);
...
hal_timer_config(int timer_num, uint32_t freq_hz)
{
    ...
    if (timer_num == 5) {
        ...
        hal_NVIC_EnableIRQ(bsptimer->tmr_irq_num);
