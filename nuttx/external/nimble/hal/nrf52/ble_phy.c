 ...
 * Change regular channels ch4 and ch5 to ch14 and ch15, use continuous ppi.
 * Regular channels: CH14, CH15 and optionally CH17, CH18, CH19
 *  - CH15 = cancel wfr timer on address match
 *  - CH16 = disable radio on wfr timer expiry
 ...
 */
#define NONE_NEST_INT_TIMER_WAR
#ifdef NONE_NEST_INT_TIMER_WAR
/*
 * 1. NRF use NRF_TIMER0->CC[0] to set compare time, and set NRF_TIMER0->EVENTS_COMPARE[0] = 0 to
 *    enable compare event, PPI setting will connect compare event to TXEN/RXEN.
 * 2. First bug is that, after the NRF_TIMER0->EVENTS_COMPARE[0] being set, at least give HW 3us
 *    to make it works, this experience value, we give 5us to ensure we will not miss this event.
 * 3. Some times other interrput(depends on the OS) will take some time during the compare set
 *    and TX transfer set,this case will case TX in wrong status, so for TX we will give a
 *    experience time between compare set and TX transfer set, it at least 30us.
 */

#define TIMER_COMP_OFFSET      (8)
#define TIME_CMP_TX_SET_GAP    (30)
#endif
...
ble_phy_wfr_enable(int txrx, uint8_t tx_phy_mode, uint32_t wfr_usecs)
{
    ...
    NRF_PPI->CHENSET = (PPI_CHEN_CH15_Msk | PPI_CHEN_CH16_Msk);
    ...
    if (NRF_TIMER0->CC[1] > NRF_TIMER0->CC[3]) {
        NRF_PPI->CHENCLR = PPI_CHEN_CH15_Msk | PPI_CHEN_CH16_Msk;
...
#ifdef NONE_NEST_INT_TIMER_WAR
        {
            /* Ble phy use cc2 to record tx end time by using ppi to set capture[2],When radio tx end,
             * cc2 will save the time, this INT should be excuted immediately to set next rx time.
             * but nuttx now not support nest INT, the this INT will be delayed by other INT, so when we
             * running here, old cc2 time is pass long time, rx_time should also passed, compare int will
             * not be trigger anymore, LL SM stopped. We need re-get current time.
             */
            uint32_t cur_cnt, start_cnt;

            /* 1. Trigger TIMER0 save current counter to CC2
             * 2. Wait for 4 clock circle to make value saved in CC2
             * 3. Read current counter and start counter.
             */
            NRF_TIMER0->TASKS_CAPTURE[2] = 1;
            for(int i = 4 ; i > 0; i--);
            cur_cnt = NRF_TIMER0->CC[2];
            start_cnt = rx_time;

            if(start_cnt > cur_cnt) {
                if((start_cnt - cur_cnt) > TIMER_COMP_OFFSET) {
                      /* Do nothing here, timer compare will be triggered. */
                }
                else {
                    rx_time += (TIMER_COMP_OFFSET - (start_cnt - cur_cnt));
                }
            } else {
                rx_time = cur_cnt + TIMER_COMP_OFFSET;
            }
        }
#endif
...
ble_phy_rx_end_isr(void)
{
...
#ifdef NONE_NEST_INT_TIMER_WAR
    {
        /* Ble phy use cc2 to record rx end time by using ppi to set capture[2],When radio rx end,
         * cc2 will save the time, this INT should be excuted immediately to set next tx time.
         * but nuttx now not support nest INT, the this INT will be delayed by other INT, so when we
         * running here, old cc2 time is pass long time, tx_time should also passed, compare int will
         * not be trigger anymore, LL SM stopped. We need re-get current time.
         */
        uint32_t cur_cnt, start_cnt;
        ...
        if(start_cnt > cur_cnt) {
            if((start_cnt - cur_cnt) > TIMER_COMP_OFFSET) {
                ...
                tx_time += (TIMER_COMP_OFFSET - (start_cnt - cur_cnt));
            }
        } else {
            tx_time = cur_cnt + TIMER_COMP_OFFSET;
        }
    }
#endif
    ...
    if ((NRF_TIMER0->CC[3] + TIME_CMP_TX_SET_GAP) > NRF_TIMER0->CC[0]) {
...
ble_phy_dbg_time_setup(void)
{
    ...
    NRF_PPI->FORK[15].TEP = (uint32_t)&(NRF_GPIOTE->TASKS_CLR[gpiote_idx]);
    NRF_PPI->FORK[16].TEP = (uint32_t)&(NRF_GPIOTE->TASKS_CLR[gpiote_idx]);
#endif
}
...
ble_phy_init(void)
{
    ...
    /*
     * PPI setup.
     * Channel 15: Captures TIMER0 in CC[3] when EVENTS_ADDRESS occurs. Used
     *            to cancel the wait for response timer.
     * Channel 16: TIMER0 CC[3] to TASKS_DISABLE on radio. This is the wait
     *            for response timer.
     */
    NRF_PPI->CH[15].EEP = (uint32_t)&(NRF_RADIO->EVENTS_ADDRESS);
    NRF_PPI->CH[15].TEP = (uint32_t)&(NRF_TIMER0->TASKS_CAPTURE[3]);
    NRF_PPI->CH[16].EEP = (uint32_t)&(NRF_TIMER0->EVENTS_COMPARE[3]);
    NRF_PPI->CH[16].TEP = (uint32_t)&(NRF_RADIO->TASKS_DISABLE);

    /* Set isr in vector table and enable interrupt */
    hal_NVIC_SetPriority(RADIO_IRQn, NVIC_SYSH_MAXNORMAL_PRIORITY);
    hal_NVIC_SetVector(RADIO_IRQn, (uint32_t)ble_phy_isr);
...
ble_phy_tx(ble_phy_tx_pducb_t pducb, void *pducb_arg, uint8_t end_trans)
{
     ...
     * all.
     */
    NRF_PPI->CHENCLR = PPI_CHEN_CH15_Msk | PPI_CHEN_CH16_Msk | PPI_CHEN_CH23_Msk |
                       PPI_CHEN_CH25_Msk;
...
ble_phy_disable_irq_and_ppi(void)
{
    ...
    NRF_RADIO->SHORTS = 0;
    NRF_RADIO->TASKS_DISABLE = 1;
    NRF_PPI->CHENCLR = PPI_CHEN_CH15_Msk | PPI_CHEN_CH16_Msk | PPI_CHEN_CH20_Msk |
          PPI_CHEN_CH21_Msk | PPI_CHEN_CH23_Msk |
          PPI_CHEN_CH25_Msk | PPI_CHEN_CH31_Msk;
    hal_NVIC_ClearPendingIRQ(RADIO_IRQn);
