...
extern void bletest_completed_bytes(uint16_t handle, uint8_t len, bool retrans);
#endif
...
#if (BLETEST_THROUGHPUT_TEST == 1)
                bletest_completed_bytes(connsm->conn_handle, 0, TRUE);
#endif
...
#if (BLETEST_THROUGHPUT_TEST == 1)
                    bletest_completed_bytes(connsm->conn_handle, txhdr->txinfo.pyld_len, FALSE);
#endif
