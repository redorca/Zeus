...
/*The stack size should 2K, less will case stack overflow.*/
#define BLE_HS_STACK_SIZE   (2048)
...
/** Task for ble host only. */
struct os_task g_ble_hs_task;

/* Queue for host task only. */
static struct ble_npl_eventq ble_host_evq;
 ...
 * Host task.
 *
 * This is the task that runs the Host task.
 *
 * @param arg
 */
...
ble_host_task(void *arg)
{
    struct ble_npl_event *ev;
    while (1) {
        ev = ble_npl_eventq_get(&ble_host_evq, BLE_NPL_TIME_FOREVER);
        assert(ev);
...
ble_hs_init(void)
{
    ...
    /* Initialize the host event queue */
    ble_npl_eventq_init(&ble_host_evq);

    /* Initialize the host task */
    os_task_init(&g_ble_hs_task, "ble_hs", ble_host_task, NULL,
                 MYNEWT_VAL(BLE_HS_PRIO), OS_WAIT_FOREVER, NULL,
                 BLE_HS_STACK_SIZE);

    /* ble_hs_evq_set((struct ble_npl_eventq *)os_eventq_dflt_get());*/
    ble_hs_evq_set(&ble_host_evq);
     ...
     */
    ble_npl_eventq_put((struct ble_npl_eventq *)os_eventq_dflt_get(), &ble_hs_ev_start_stage1);
