...
/**
 * Check if entry with this peer address is bonded or not.
 *
 * @param peer_id_addr          Entry with this peer address.
 *
 * @return                      True means bonded;
 *                              False means not bonded.
 *                              Other nonzero on error.
 */
int
ble_store_util_bonded_check(uint16_t conn_handle)
{
    ...
    for(i = 0; i < num_peers; i++) {
        if(0 == ble_addr_cmp(&peer_id_addrs[i], &(addrs.peer_id_addr))) {
            return TRUE;
        }
    }
