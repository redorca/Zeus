...
#include "../dfu/nrf_dfu_req_handler.h"
#include "../dfu/nrf_dfu_transport.h"
...
#define FIRST_CONN_PARAMS_UPDATE_DELAY       100              /**< Time from the Connected event to first time sd_ble_gap_conn_param_update is called (100 milliseconds). */
#define NEXT_CONN_PARAMS_UPDATE_DELAY        500             /**< Time between each call to sd_ble_gap_conn_param_update after the first call (500 milliseconds). */
...
static ble_dfu_t
m_dfu;                                                                   /**< Structure used to identify the Device Firmware Update service. */
static uint16_t
m_pkt_notif_target;                                                      /**< Number of packets of firmware data to be received before transmitting the next Packet Receipt Notification to the DFU Controller. */
static uint16_t
m_pkt_notif_target_cnt;                                                  /**< Number of packets of firmware data received after sending last Packet Receipt Notification or since the receipt of a @ref BLE_DFU_PKT_RCPT_NOTIF_ENABLED event from the DFU service, which ever occurs later.*/
...
static uint8_t
m_notif_buffer[MAX_RESPONSE_LEN];                                                    /**< Buffer used for sending notifications to peer. */
...
static uint32_t ble_stack_init(bool init_softdevice)
{
  ...
  NRF_LOG_INFO("Enabling interrupts!\r\n");
  irq_ret = (uint32_t) up_irq_save();
  for (int i = 2; i < 54; i++)
    {
      up_enable_irq(i);
    }
