...
typedef struct
{
  ...
  pm_store_token_t
  store_token;           /**< Token given by Peer Data Storage when a flash write has been successfully requested. */
...
uint8_t             store_flash_full :
  1;  /**< Flag indicating that the buffer was attempted written to flash, but a flash full error was returned and the operation should be retried after room has been made. */
uint8_t             store_busy       :
  1;  /**< Flag indicating that the buffer was attempted written to flash, but a busy error was returned and the operation should be retried. */
} pdb_buffer_record_t;
...
static uint32_t
m_n_writes;                                     /**< The number of pending (Not yet successfully requested in Peer Data Storage) store operations. */
...
{
#if defined(RETURN_ERROR_IF_MODULE_UNINITIALIZED)
...
static void peer_data_point_to_buffer(pm_peer_data_t *p_peer_data, pm_peer_data_id_t data_id, uint8_t *p_buffer_memory,
                                      uint16_t n_bufs)
{
...
static void peer_data_const_point_to_buffer(pm_peer_data_const_t *p_peer_data, pm_peer_data_id_t data_id,  uint8_t *p_buffer_memory,
                                            uint32_t n_bufs)
{
