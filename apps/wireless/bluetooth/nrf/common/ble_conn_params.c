...
#define CONN_PARAMS_UPDATE_DELAY 1500 /* 1500ms */
...
static uint8_t
m_update_count;           /**< Number of Connection Parameter Update messages that has currently been sent. */
...
timer_t m_conn_params_timer_id = NULL;
...
void update_timeout_handler(void)
{
...
uint32_t ble_conn_params_init(const ble_conn_params_init_t *p_init)
{
  ...
  err_code = app_timer_create(&m_conn_params_timer_id, update_timeout_handler);
  if (err_code < 0)
    {
  ...
  if (!is_conn_params_ok(&m_current_conn_params))
    {
      ...
      if (m_change_param)
        ...
        {
          err_code = app_timer_start(m_conn_params_timer_id, CONN_PARAMS_UPDATE_DELAY, APP_TIMER_MODE_SINGLE_SHOT);
          if ((err_code != NRF_SUCCESS) && (m_conn_params_config.error_handler != NULL))
            {
              m_conn_params_config.error_handler(err_code);
            }
...
uint32_t ble_set_preferred_conn_paramter(ble_gap_conn_params_t *ppcp)
{
  uint32_t err_code;
  m_preferred_conn_params = *ppcp;
  err_code = sd_ble_gap_ppcp_set(&m_preferred_conn_params);
  return err_code;
}
