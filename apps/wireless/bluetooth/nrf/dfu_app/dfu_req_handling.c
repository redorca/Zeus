...
#include <stdint.h>
#include <stdbool.h>
#include "../bootloader/dfu/nrf_dfu_req_handler.h"
#include "dfu_req_handling.h"
#include "../bootloader/dfu/nrf_dfu.h"
#include "../bootloader/dfu/nrf_dfu_types.h"
#include "../bootloader/dfu/nrf_dfu_settings.h"
#include "../bootloader/dfu/nrf_dfu_transport.h"
#include "../bootloader/dfu/nrf_dfu_utils.h"
#include "../bootloader/dfu/nrf_dfu_flash.h"
#include "../bootloader/ble_dfu/nrf_ble_dfu.h"
...
static uint8_t
m_current_data_buffer;                  /**< Index of the current data buffer. Must be between 0 and FLASH_BUFFER_CHUNK_COUNT - 1. */
static uint32_t
m_flash_operations_pending;             /**< A counter holding the number of pending flash operations. This will prevent flooding of the buffers. */
...
static bool
m_valid_init_packet_present;                /**< Global variable holding the current flags indicating the state of the DFU process. */
...
static nrf_dfu_res_code_t dfu_handle_prevalidate(dfu_signed_command_t const *p_command, pb_istream_t *p_stream, uint8_t *p_init_cmd,
  ...
  switch (p_req->req_type)
    {
      case NRF_DFU_OBJECT_OP_CREATE:
        ...
        if ((s_dfu_settings.progress.firmware_image_offset_last + p_req->object_size) > m_firmware_size_req)
          {
            NRF_LOG_ERROR("Trying to create an object of size %d, when offset is 0x%08x and firmware size is 0x%08x\r\n", p_req->object_size,
                          s_dfu_settings.progress.firmware_image_offset_last, m_firmware_size_req);
            return NRF_DFU_RES_CODE_OPERATION_NOT_PERMITTED;
          }
        ...
        m_flash_operations_pending++;
        if (nrf_dfu_flash_erase((uint32_t *)(m_firmware_start_addr + s_dfu_settings.progress.firmware_image_offset),
                                CEIL_DIV(p_req->object_size, CODE_PAGE_SIZE), dfu_data_write_handler) != FS_SUCCESS)
          {
            m_flash_operations_pending--;
        ...
        NRF_LOG_INFO("Creating object with size: %d. Offset: 0x%08x, CRC: 0x%08x\r\n", s_dfu_settings.progress.data_object_size,
                     s_dfu_settings.progress.firmware_image_offset, s_dfu_settings.progress.firmware_image_crc);
      ...
      case NRF_DFU_OBJECT_OP_WRITE:
        ...
        if ((p_req->req_len + s_dfu_settings.progress.firmware_image_offset - s_dfu_settings.progress.firmware_image_offset_last) >
            s_dfu_settings.progress.data_object_size)
          {
        ...
        if (m_data_buf_pos + p_req->req_len < FLASH_BUFFER_CHUNK_LENGTH)
        ...
        else
          {
            ...
            ++m_flash_operations_pending;
            if (nrf_dfu_flash_store(p_write_addr, (uint32_t *)&m_data_buf[m_current_data_buffer][0], CEIL_DIV(m_data_buf_pos, 4),
                                    dfu_data_write_handler) == FS_SUCCESS)
              {
