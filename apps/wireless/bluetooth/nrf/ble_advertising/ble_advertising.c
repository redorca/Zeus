...
static bool
m_advertising_start_pending;                 /**< Flag to keep track of ongoing operations in flash. */

static ble_adv_evt_t
m_adv_evt;                                   /**< Advertising event propogated to the main application. The event is either a transaction to a new advertising mode, or a request for whitelist or peer address. */

static ble_adv_mode_t
m_adv_mode_current;                          /**< Variable to keep track of the current advertising mode. */
static ble_adv_modes_config_t
m_adv_modes_config;                          /**< Struct to keep track of disabled and enabled advertising modes, as well as time-outs and intervals.*/

static ble_gap_addr_t
m_peer_address;                              /**< Address of the most recently connected peer, used for direct advertising. */
static bool
m_peer_addr_reply_expected;                  /**< Flag to verify that peer address is only set when requested. */

static ble_advdata_t
m_advdata;                                   /**< Used by the initialization function to set name, appearance, and UUIDs and advertising flags visible to peer devices. */
static ble_advdata_manuf_data_t        m_manuf_specific_data;                       /**< Manufacturer specific data structure*/
static uint8_t
m_manuf_data_array[BLE_GAP_ADV_MAX_SIZE];    /**< Array to store the Manufacturer specific data*/
...
static ble_advertising_evt_handler_t
m_evt_handler;                               /**< Handler for the advertising events. Can be initialized as NULL if no handling is implemented on in the main application. */
static ble_advertising_error_handler_t
m_error_handler;                             /**< Handler for the advertising error events. */

static bool
m_whitelist_temporarily_disabled;            /**< Flag to keep track of temporary disabling of the whitelist. */
...
static bool whitelist_has_entries(void)
{
 ...
 */
static bool flash_access_in_progress(void)
{
  uint32_t count;
...
void ble_adv_mode_set(const ble_adv_modes_config_t *p_adv_modes_config)
{
  if (p_adv_modes_config == NULL)
    {
      NRF_LOG_ERROR("\n ADV set NULL Pointer ERROR...\n");
      return;
    }
  memcpy(&m_adv_modes_config, p_adv_modes_config, sizeof(ble_adv_modes_config_t));
}
